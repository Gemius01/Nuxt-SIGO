    ...
    <v-dialog v-model="dialogDelete" persistent max-width="350">
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue" flat @click="deleteCategoriaH">Eliminar</v-btn>
  ...
  <!-- Dialog Agregar Tipo -->
          ...
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md12>
                <v-text-field label="Nombre"  :rules="textoRules" :counter="20" name="nombreCategoriaH" v-model="addItem.nombre" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
    ...
    <!-- Dialog Editar Tipo -->
       <v-dialog v-model="dialogEdit" max-width="500px">
        <v-form @submit.prevent="editCategoriaH" ref="fEditarCategoriaH">
      <v-card>
        <v-card-title>
    ...
    <v-card>
    <v-card-title>
        <h1>LISTA CATEGORÍA HERRAMIENTA</h1>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
          label="Buscar"
  ...
  export default {
    ...
    methods: {
      initialize () { // Función que recarga los datos de la Tabla mediante request a la API REST
        axios.get(config.API_LOCATION + `/bodega/categoria/tipo/herramienta`) // petición GET a Tipo para traer a todos los objetos "tipo"
          .then((response) => {
      ...
      agregarCategoriaH (e) { // función para agregar un nuevo Tipo
        var categoria = e.target.elements.nombreCategoriaH.value
        var tipo = 1
        if (this.$refs.fAgregarCategoriaH.validate()) {
      ...
      editCategoriaH (e) { // función para editar el tipo
        ...
        if (this.$refs.fEditarCategoriaH.validate()) {
      ...
      deleteCategoriaH () { // función para eliminar un tipo
