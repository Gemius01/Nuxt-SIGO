<template>
  ...
  <div id="reporte">
     <v-card>
    <v-card-title>
        <h1>Reporte Herramienta Sin Stock</h1>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
          label="Buscar"
...
<script>
  ...
  export default {
    ...
    methods: {
      datosReporte () {
        axios.get(config.API_LOCATION + '/bodega/item/reportHerramienta0') // petición GET a Marca para traer a todos los objetos "Marca"
          .then((response) => {
