...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      cargarSelectCat () {
        axios.get(config.API_LOCATION + `/bodega/categoria/tipo/herramienta/`) // petición GET a Categoria para traer a todos los objetos "categoria"que contengan como tipo "insumo"
          .then((response) => {
      ...
      initialize () { // Función que recarga los datos de la Tabla mediante request a la API REST
        axios.get(config.API_LOCATION + `/bodega/subcategoria/tipo/herramienta/`) // petición GET a Subcategoría para traer todos los objetos "Subcategoría" que contengan como tipo insumo
          .then((response) => {
            for (var i = response.data.length; i >= 0; i--) {
