    ...
    <v-dialog v-model="dialogDelete" persistent max-width="350">
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue" flat @click="deleteMarca">Eliminar</v-btn>

        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Termino Dialog Eliminar Marca-->
  <!-- Dialog Agregar Marca -->
    <v-dialog v-model="dialogAdd" max-width="500px">
      <v-btn dark color="primary"  slot="activator" >Agregar Marca</v-btn>
          ...
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md12>
                <v-text-field label="Nombre"  :rules="textoRules" :counter="20" name="nombreMarca" v-model="addItem.nombre" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
    ...
    <!-- Fin Dialog Agregar Marca -->
    <!-- Dialog Editar Marca -->
       <v-dialog v-model="dialogEdit" max-width="500px">
        <v-form @submit.prevent="editMarca" ref="fEditarMarca">
      <v-card>
        <v-card-title>
    ...
    <!-- Fin Dialog Editar Marca -->
    <!-- Dialog Detalle Marca -->
       <v-dialog v-model="dialogDetail" max-width="500px">
        <v-form @submit.prevent="">
      <v-card>
 
          <v-card-title><h1> Detalle de Categoría Marca </h1></v-card-title>
          <v-divider></v-divider>
          ...
          <v-list dense >
            ...
            <v-list-tile class="hoverMouse">
              <v-list-tile-title>Nombre Marca</v-list-tile-title>
              <v-list-tile-title class="text-lg-center">:</v-list-tile-title>
...
<script>
  ...
  export default {
    ...
    data: () => ({
      ...
      dialogAdd: false, // prop para abrir y cerrar modal de Agregar Marca
      dialogEdit: false, // prop para abrir y cerrar modal de Editar Marca
      dialogDetail: false, // prop para abrir y cerrar modal de Detalle Marca
      dialogDelete: false, // prop para abrir y cerrar modal de Delete Marca
    ...
    methods: {
      initialize () { // Función que recarga los datos de la Tabla mediante request a la API REST
        axios.get(config.API_LOCATION + `/bodega/marca/`) // petición GET a Marca para traer a todos los objetos "Marca"
          .then((response) => {
      ...
      agregarMarca (e) { // función para agregar un nuevo Marca
        var Marca = e.target.elements.nombreMarca.value
      ...
      editMarca (e) { // función para editar el Marca
        ...
        if (this.$refs.fEditarMarca.validate()) {
          axios.put(config.API_LOCATION + '/bodega/marca/' + id + '', {// petición put para editar el Marca
            nombre: '' + nombre + ''
          })
            .then(response => {
      ...
      modalDelete (item) { // función para editar el Marca
        this.deleteIndex = this.items.indexOf(item) // obtener posición del array
        this.deleteItem = Object.assign({}, item)
        this.dialogDelete = true
      },
      deleteMarca () { // función para eliminar un Marca
        var id = this.deleteItem.id // id del objeto a eliminar
        console.log(this.items.length)
        axios.delete(config.API_LOCATION + '/bodega/giro/' + id + '') // petición DELETE a Marca para poder eliminar un Marca
          .then(response => {
