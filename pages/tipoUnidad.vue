          ...
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md12>
                <v-text-field label="nombre"  :rules="textoRules" :counter="20" name="nombreUnidad" v-model="addItem.nombre" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
  ...
  export default {
    ...
    methods: {
      initialize () { // Función que recarga los datos de la Tabla mediante request a la API REST
        axios.get(config.API_LOCATION + `/bodega/tipo_unidad/`) // petición GET a Unidad para traer a todos los objetos "Unidad"
          .then((response) => {
      ...
      deleteUnidad () { // función para eliminar un Unidad
        ...
        axios.delete(config.API_LOCATION + '/bodega/tipo_unidad/' + id + '') // petición DELETE a Unidad para poder eliminar un Unidad
          .then(response => {
