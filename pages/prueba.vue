 ...
 <v-snackbar
    ...
    <v-dialog v-model="dialogDelete" persistent max-width="350">
      ...
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="blue" flat @click="deleteTipo">Eliminar</v-btn>
  ...
  <!-- Dialog Agregar Tipo -->
    <v-dialog v-model="dialogAdd" max-width="500px">
      <v-btn dark color="primary"  slot="activator" >Agregar Tipo</v-btn>
            <v-form @submit.prevent="agregarTipo" v-model="valid" ref="fAgregarTipo" lazy-validation>
      <v-card>
        <v-card-title>
          <span class="headline">Agregar Tipo</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md12>
                <v-text-field label="Nombre"  :rules="textoRules" :counter="20" name="nombreTipo" v-model="addItem.nombre" required></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
    ...
    <!-- Dialog Editar Tipo -->
       <v-dialog v-model="dialogEdit" max-width="500px">
        <v-form @submit.prevent="editTipo" ref="fEditarTipo">
      <v-card>
        <v-card-title>
  ...
  export default {
    ...
    methods: {
      ...
      initialize () { // Función que recarga los datos de la Tabla mediante request a la API REST
        axios.get(config.API_LOCATION + `/bodega/tipo/`) // petición GET a Tipo para traer a todos los objetos "tipo"
          .then((response) => {
      ...
      agregarTipo (e) { // función para agregar un nuevo Tipo
        var tipo = e.target.elements.nombreTipo.value
        if (this.$refs.fAgregarTipo.validate()) {
          axios.post(config.API_LOCATION + '/bodega/tipo/', { // petición POST a Tipo para agregar
            nombre: '' + tipo + ''
          })
            .then((response) => {
              this.items.push(response.data) // push al array de items
              this.$refs.fAgregarTipo.reset()
      ...
      editTipo (e) { // función para editar el tipo
        ...
        if (this.$refs.fEditarTipo.validate()) {
          axios.put(config.API_LOCATION + '/bodega/tipo/' + id + '', {// petición put para editar el tipo
            nombre: '' + nombre + ''
          })
            .then(response => {
      ...
      deleteTipo () { // función para eliminar un tipo
        var id = this.deleteItem.id // id del objeto a eliminar
        console.log(this.items.length)
        axios.delete(config.API_LOCATION + '/bodega/tipo/' + id + '') // petición DELETE a tipo para poder eliminar un tipo
          .then(response => {
