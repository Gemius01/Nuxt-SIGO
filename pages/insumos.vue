 ...
 <v-snackbar
  ...
  <!-- Dialog Agregar Tipo -->
    <v-dialog v-model="dialogAdd" max-width="500px" >
      <v-btn dark color="primary"  slot="activator" >Agregar Insumos</v-btn>
            <v-form @submit.prevent="agregarInsumos" v-model="valid" ref="fAgregarInsumos" lazy-validation>
      <v-card>
        <v-card-title>
          <span class="headline">Agregar Insumos</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
                <v-flex xs12>
                <v-text-field label="Nombre"  :rules="textoRules" :counter="20" name="nombreInsumo" v-model="addItem.nombre" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                <v-text-field label="Modelo"  :rules="textoRules" :counter="20" name="modeloInsumo" v-model="addItem.modelo" required></v-text-field>
                </v-flex>
                <v-flex xs12>
                <v-text-field label="Stock Critico" :rules="numberRule" :counter="20" name="stockCriticoInsumo" v-model="addItem.stockCritico" type="number" required></v-text-field>
                </v-flex>
...
<v-dialog v-model="dialog3" max-width="500px">
    ...
    <!-- Dialog Editar Tipo -->
       <v-dialog v-model="dialogEdit" max-width="500px">
        <v-form @submit.prevent="editInsumo" ref="fEditarInsumos">
      <v-card>
        <v-card-title>
          <span class="headline">Editar Insumo</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
  ...
  export default {
    ...
    data: () => ({
      ...
      headers: [ // Encabezados de  la tabla
        ...
        { text: 'Stock', value: 'stock', width: '5%', align: 'center' },
    ...
    methods: {
      ...
      cargarSelectMarca () {
        axios.get(config.API_LOCATION + `/bodega/marca/`) // petición GET a Categoria para traer a todos los objetos "categoria"que contengan como tipo "insumo"
          .then((response) => {
      ...
      editInsumo (e) { // función para editar la Subcategoría
        ...
        console.log(subCategoria)
        if (this.$refs.fEditarInsumos.validate()) {
