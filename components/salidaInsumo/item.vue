...
export default {
  ...
  methods: {
    ...
    agregarItemPrestado () {
      this.inputItem.push({idInput: 'selectItemPrestamo' + this.count + '', idCantidad: 'cantidad' + this.count + ''})
      this.$emit('clicked', {idInput: 'selectItemPrestamo' + this.count + '', idCantidad: 'cantidad' + this.count + ''})
      this.count = this.count + 1
    },
    cargarSelectCat () {
      axios.get(config.API_LOCATION + '/bodega/item/insumo') // petición GET a Categoria para traer a todos los objetos "categoria"que contengan como tipo "insumo"
        .then((response) => {
